class Yacht {
  ...
  setup() {
    this.img = loadImage("assets/sailboatLeft.png");
  }
  ...
  sail(course) {
    ...
    if (course == "upLeft") {
      ...
      this.img = loadImage("assets/sailboatRight.png");
      this.x = (this.x + (1*SAILSPEED));
      this.y = (this.y - (1*SAILSPEED));
    }
    ...
    //Control if the sailboat tries to cross the start line before countdown
    if (!(regattaStarted) && (this.y > game.buoy2.y) && (this.y < game.raceCommittee.y) && (this.x > game.buoy2.x) && (this.x < game.raceCommittee.x)) {
      gameCredits = gameCredits - 20;
      creditsNode.innerHTML = "Game Credits "+ gameCredits;
      fill("orange");
      text("Wait behind the start line",this.x-400,this.y-30);
      game.yacht1.y = game.yacht1.y + YACHTSIZE;
      noLoop();
    ...
    if ((regattaStep === 0) && (this.x < game.buoy2.x) && (this.y < game.buoy2.y)) {
      text("Sail to BUOY 3",WIDTH/3,HEIGHT/2);
      ...
      text("Sail to BUOY 1",WIDTH/3,HEIGHT/2);
      ...
      text("Sail to BUOY 2",WIDTH/3,HEIGHT/2);
